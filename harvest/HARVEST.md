...
- [ ] **Frozen synthetic reference exists** for every battery prompt in play (`baseline/exemplars/<prompt-id>/exemplar.md`), generated by a different generation than the targets and **human-approved** (`baseline/exemplars/README.md`). This is the baseline the tic-finder diffs against — the one hard prerequisite.
...
- [ ] Judge model recorded (blind-judge leg) — a **different generation** than any target, and different from the exemplar generator where possible.
